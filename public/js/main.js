...
const loadRestaurantInfo = async () => {
  try {
    ...
    if (brand) {
      if (data.logo) {
        brand.innerHTML = `<img src="${data.logo}" alt="${data.name || 'Logo'}" style="max-height: 38px; max-width: 160px; display: block; object-fit: contain; border-radius: 4px;">`;
      } else {
        brand.textContent = data.name || 'Menü';
      }
    }
...
const renderMenuItem = (p, options = {}) => {
  ...
  const prep =
    p.preparation_time != null
      ...
      : '';
  return `
    <div class="menu-item${chefPick ? ' menu-item-chef-pick' : ''}" role="button" tabindex="0" data-slug="${escapeHtml(p.slug)}">
      ${img}
      <div class="item-info">
        ${badge}
...
const loadClientBanners = async () => {
  ...
  const pagination = document.getElementById('bannersPagination');
  if (!section || !slider) return;

  try {
    const response = await fetch(`${API_BASE}/banners`);
    if (!response.ok) throw new Error('Banners fetch failed');
    ...
    slider.innerHTML = banners.map(b => {
      const linkAttr = b.link ? ` data-link="${escapeHtml(b.link)}"` : '';
      const titleHtml = b.title ? `<div class="banner-title">${escapeHtml(b.title)}</div>` : '';
      const descHtml = b.description ? `<div class="banner-desc">${escapeHtml(b.description)}</div>` : '';
      const overlayHtml = (titleHtml || descHtml) 
        ? `<div class="banner-overlay">${titleHtml}${descHtml}</div>`
        : '';
      return `
        <div class="banner-slide"${linkAttr}>
          <img src="${escapeHtml(b.image)}" alt="${escapeHtml(b.title || '')}" onerror="this.onerror=null; this.src='/uploads/ana-yemek-banner.webp';">
          ${overlayHtml}
        </div>
      `;
    }).join('');
    ...
    if (banners.length > 1) {
      ...
      pagination.innerHTML = banners.map((_, idx) => `
        <div class="pagination-dot${idx === 0 ? ' active' : ''}" data-idx="${idx}"></div>
      `).join('');
      setupBannersSlider(banners.length);
    } else {
...
const setupBannersSlider = (slideCount) => {
  const slider = document.getElementById('bannersSlider');
  const dots = document.querySelectorAll('#bannersPagination .pagination-dot');
